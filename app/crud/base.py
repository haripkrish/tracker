...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_by_field(self, db: Session, *, field_name: str, field_value: str) -> ModelType | None:
        obj_data = db.execute(select(self.model).filter_by(**{field_name: field_value})).one_or_none()
        if obj_data:
            return obj_data[0]
